...
using KonigLabs.FantaEmotion.SDKData.Structs;
...
namespace EDSDKLib
{
    public class SDKHandler : IDisposable
    {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            var buffer = new byte[imageBlobLength];
        ...
        public List<Camera> GetCameraList()
        {
            ...
            var outCamList = new List<Camera>();
            for (var i = 0; i < c; i++)
            {
                IntPtr cptr;
        ...
        public byte[] DownloadImage(IntPtr objectPointer)
        {
            ...
            var dirInfo = new DirectoryItemInformation();
            _returnValueManager.HandleFunctionReturnValue(EdsdkInvokes.GetDirectoryItemInformation(objectPointer, out dirInfo));

            //check the extension. Raw data cannot be read by the bitmap class
            var ext = Path.GetExtension(dirInfo.FileName).ToLower();
            if (ext == ".jpg" || ext == ".jpeg")
            {
        ...
        private Bitmap GetImage(IntPtr imgStream, ImageSource imageSource)
        {
            ...
            try
            {
                ...
                var outputSize = new KonigLabs.FantaEmotion.SDKData.Structs.Size();
                outputSize.Width = imageInfo.EffectiveRectangle.Width;
                outputSize.Width = imageInfo.EffectiveRectangle.Height;

                //calculate amount of data
                var datalength = outputSize.Height * outputSize.Width * 3;
        ...
        public List<int> GetSettingsList(PropertyId propId)
        {
            if (MainCamera.Ref != IntPtr.Zero)
            {
                    ...
                    || propId == PropertyId.Tv || propId == PropertyId.MeteringMode || propId == PropertyId.ExposureCompensation)
                {
                    ...
                    var des = new PropertyDescription();
        ...
        public T GetStructSetting<T>(uint PropID) where T : struct
        {
            if (MainCamera.Ref != IntPtr.Zero)
            {
                ...
                var bufferSize = Marshal.SizeOf(structureType);
                ...
                try
                {
                    ...
                    var data = (T)Marshal.PtrToStructure(ptr, structureType);
                    return data;
                }
                finally
                {
        ...
        public void SetStringSetting(uint PropID, string Value)
        {
            if (MainCamera.Ref != IntPtr.Zero)
            {
                ...
                var propertyValueBytes = Encoding.ASCII.GetBytes(Value + '\0');
                var propertySize = propertyValueBytes.Length;
        ...
        private Rectangle GetEvfZoomRect(IntPtr imgRef)
        {
            ...
            var err = (ReturnValue) EdsdkInvokes.GetPropertyData(imgRef, (uint) PropertyId.LiveViewZoomRectangle, 0, size, ptr);
            var rect = (Rectangle)Marshal.PtrToStructure(ptr, typeof(Rectangle));
            Marshal.FreeHGlobal(ptr);
        ...
        {
            var size = Marshal.SizeOf(typeof(Size));
            var ptr = Marshal.AllocHGlobal(size);
            var err = (ReturnValue) EdsdkInvokes.GetPropertyData(imgRef, (uint) PropertyId.LiveViewCoordinateSystem, 0, size, ptr);
            _returnValueManager.HandleFunctionReturnValue(err);
            var coord = (Size)Marshal.PtrToStructure(ptr, typeof(Size));
            Marshal.FreeHGlobal(ptr);
        ...
        private Point GetEvfPoints(uint propId, IntPtr imgRef)
        {
            ...
            var ptr = Marshal.AllocHGlobal(size);
            var error = (ReturnValue) EdsdkInvokes.GetPropertyData(imgRef, propId, 0, size, ptr);
            _returnValueManager.HandleFunctionReturnValue(error);
            var data = (Point)Marshal.PtrToStructure(ptr, typeof(Point));
            Marshal.FreeHGlobal(ptr);
        ...
        public void SetManualWBEvf(ushort x, ushort y)
        {
            if (IsLiveViewOn)
            {
                SendSDKCommand(cam =>
                {
                    ...
                    var xa = BitConverter.GetBytes(x);
                    var ya = BitConverter.GetBytes(y);
                    var coord = BitConverter.ToUInt32(new[] { xa[0], xa[1], ya[0], ya[1] }, 0);
                    //send command to camera
        ...
        public CameraFileEntry GetAllEntries()
        {
            ...
            var MainEntry = new CameraFileEntry("Camera", true);
            ...
            var VolumeEntries = new List<CameraFileEntry>();
        ...
        public void UILock(bool lockState)
        {
            SendSDKCommand(cam =>
            {
                if (lockState) _returnValueManager.HandleFunctionReturnValue(EdsdkInvokes.SendStatusCommand(cam.Ref, (uint) CameraState.UILock, 0));
        ...
        private CameraFileEntry[] GetChildren(IntPtr ptr)
        {
            ...
            if (ChildCount > 0)
            {
                ...
                var mainEntry = new CameraFileEntry[ChildCount];
                for (var i = 0; i < ChildCount; i++)
                {
                    ...
                    else
                    {
                        ...
                        var retval = GetChildren(childPtr);
