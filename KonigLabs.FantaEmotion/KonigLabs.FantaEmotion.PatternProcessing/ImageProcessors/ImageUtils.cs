...
using KonigLabs.FantaEmotion.PatternProcessing.ProcessingViews;
...
namespace KonigLabs.FantaEmotion.PatternProcessing.ImageProcessors
{
    public class ImageUtils
    {
        public byte[] GetCaptureForInstagramControl(byte[] imageForInstagram, string publishAuthorName, DateTime timeUpdate, byte[] profilePictureData)
        {
            ...
            var dpiX = 96;
            var dpiY = 96;
            var bounds = VisualTreeHelper.GetDescendantBounds(defaultTemplate);
            ...
            {
                var vb = new VisualBrush(defaultTemplate);
            ...
            using (var rectangleResultImage = new Bitmap((int)bounds.Width, (int)bounds.Height))
            {
                using (var canvas = Graphics.FromImage(rectangleResultImage))
                {
                    canvas.InterpolationMode = InterpolationMode.HighQualityBicubic;

                    using (var imageStream = new MemoryStream(captureImageData))
                    {
                        var squereResultImage = Image.FromStream(imageStream);
        ...
        public Bitmap ResizeImage(Image image, int width, int height, float hResolution, float vResolution)
        {
            ...
            var target = new Bitmap(cropRect.Width, cropRect.Height, image.PixelFormat);
        ...
        {
            var fractionalPercentage = (height / (double)img.Height);
